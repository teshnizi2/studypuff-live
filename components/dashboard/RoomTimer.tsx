...
import { InlineSoundChooser } from "./InlineSoundChooser";
...
export type RoomTimerProps = {
  ...
  /** Personal stats — gauge + sparkline rendered under the buttons,
   *  identical to the solo timer. Per-user, not shared. */
  todayMinutes: number;
...
export function RoomTimer({
  ...
  return (
    <div className="relative flex flex-col items-center text-ink-900">
      ...
      {/* Today's gauge — per-user. Same shape as the solo timer's gauge
          so the dashboard reads identically in room vs solo mode. */}
        ...
        <div className="flex items-baseline justify-between text-[11px] uppercase tracking-[0.28em] text-ink-700">
          ...
          <span>
            ...
            {dailyGoalMinutes && dailyGoalMinutes > 0 && (
              <span className="text-ink-700/60"> / {dailyGoalMinutes}</span>
            )}
          </span>
        </div>
        <div className="mt-2 h-[3px] w-full overflow-hidden rounded-full bg-ink-900/10">
          <div
            ...
            style={{
              width: `${
                (dailyGoalMinutes && dailyGoalMinutes > 0
                  ? Math.min(1, todayMinutes / dailyGoalMinutes)
                  : Math.min(1, todayMinutes / 60)) * 100
              }%`
            }}
          />
        </div>
      ...
      {/* Weekly sparkline — per-user, same shape as solo. */}
      ...
      {/* Sound chooser — same placement and shape as the solo timer. */}
      <InlineSoundChooser
        sound={sound}
          ...
          <div key={d.date} className="flex flex-1 flex-col items-center gap-1">
            <div
              className={`w-full rounded-md ${
                isToday ? "bg-emerald-700" : d.minutes > 0 ? "bg-emerald-700/35" : "bg-ink-900/10"
              }`}
              style={{ height: `${h}px` }}
...
// (InlineSoundChooser now lives in its own file and is shared with the
// solo TimerCircle.)
