...
export type RailItem = {
  ...
  icon: ReactNode;
  active: boolean;
  /** Click handler for modal-toggle items. Ignored when `href` is set. */
  onClick?: () => void;
  /** Small dot badge (e.g. an active room). */
  badge?: boolean;
  /** When this item opens a modal dialog (haspopup/expanded semantics) — not
      true for in-place toggles (Tasks) or navigation items (href set). */
  haspopup?: boolean;
  /** When set, the item renders as a Next.js Link to this route rather than
      a button. Use for items that own their own page (e.g. /dashboard/garden). */
  href?: string;
};

// Slim vertical icon rail pinned to the left edge of the dashboard.
// Some entries open a modal (button + onClick), others navigate to a page
// (Link + href). lg+ only; on small screens the dashboard stacks instead.
  ...
  return (
    ...
    >
      {items.map((item) => {
        ...
        const baseCx = `group relative flex h-[52px] w-[52px] flex-col items-center justify-center gap-0.5 rounded-2xl transition-all duration-200 ease-[cubic-bezier(0.34,1.56,0.64,1)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-700/50 focus-visible:ring-offset-1 focus-visible:ring-offset-cream-50 ${
          item.active
